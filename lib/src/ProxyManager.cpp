...
ProxyManager::ProxyManager(WriterManager& writer_manager, ZmqReceiver& receiver, RingBuffer& ring_buffer, 
...
void ProxyManager::notify_first_pulse_id(uint64_t pulse_id) 
{
    ...
    async(launch::async, [pulse_id, &request_address]{
        try {
            ...
            string request_call(request.str());
                ...
                cout << "[ProxyManager::notify_first_pulse_id] Sending request (" << request_call << ")." << endl;
            #endif
...
void ProxyManager::notify_last_pulse_id(uint64_t pulse_id) 
{
    ...
    try {
        ...
        string request_call(request.str());
            ...
            using namespace date;
            cout << "[" << std::chrono::system_clock::now() << "]";
            cout << "[ProxyManager::notify_last_pulse_id] Sending request (" << request_call << ")." << endl;
        #endif
...
void ProxyManager::run_writer()
{

    #ifdef DEBUG_OUTPUT
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::run_writer] Running writer";
        cout << " and output_file " << writer_manager.get_output_file();
        cout << endl;
    #endif

    boost::thread receiver_thread(&ProxyManager::receive_zmq, this);
    ...
    RestApi::start_rest_api(writer_manager, rest_port);
        ...
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::run_writer] Rest API stopped." << endl;
    #endif
    ...
    receiver_thread.join();
        ...
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::run_writer] Writer properly stopped." << endl;
    #endif
}

void ProxyManager::receive_zmq()
{
    receiver.connect();
    ...
    while (writer_manager.is_running()) {
        ...
        auto frame_data = frame.second;
            ...
            using namespace date;
            cout << "[" << std::chrono::system_clock::now() << "]";
            cout << "[ProxyManager::receive_zmq] Processing FrameMetadata"; 
        ...
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::receive_zmq] Receiver thread stopped." << endl;
    #endif
}

void ProxyManager::write_h5(size_t frames_per_file)
{
    ...
    while(writer_manager.is_running() || !ring_buffer.is_empty()) {
        ...
        if (!writer->is_data_for_current_file(received_data.first->frame_index)) {
                ...
                using namespace date;
                cout << "[" << std::chrono::system_clock::now() << "]";
                cout << "[ProxyManager::write_h5] Frame index " << received_data.first->frame_index;
        ...
        writer->write_data(raw_frames_dataset_name,
            ...
            cout << "[" << std::chrono::system_clock::now() << "]";
            cout << "[ProxyManager::write_h5] Frame index "; 
        ...
        if (header_values_type) {
            ...
            cout << "[" << std::chrono::system_clock::now() << "]";
            cout << "[ProxyManager::write_h5] Frame metadata index "; 
    ...
    if (writer->is_file_open()) {
            ...
            using namespace date;
            cout << "[" << std::chrono::system_clock::now() << "]";
            cout << "[ProxyManager::write] Writing file format." << endl;
        #endif
        ...
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::write] Closing file " << writer_manager.get_output_file() << endl;
    #endif
    ...
    writer->close_file();
        ...
        using namespace date;
        cout << "[" << std::chrono::system_clock::now() << "]";
        cout << "[ProxyManager::write] Writer thread stopped." << endl;
    #endif
...
void ProxyManager::write_h5_format(H5::H5File& file) {

    if (!writer_manager.are_all_parameters_set()) {
        using namespace date;
        std::cout << "[" << std::chrono::system_clock::now() << "]";
        std::cout << "[ProxyManager::write_h5_format] Cannot write file format - REST parameters not set."<< endl;
    ...
    try {
        ...
        using namespace date;
        std::cout << "[" << std::chrono::system_clock::now() << "]";
        std::cout << "[ProxyManager::write_h5_format] Error while trying to write file format: "<< ex.what() << endl;
    }
}
