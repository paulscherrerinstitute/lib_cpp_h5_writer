...
@app.route('/server_log/<endpoint>', methods=['GET'])
def get_server_log(endpoint):
    if request.method == 'GET':
        ...
        if debug:
            ...
            log = subprocess.run(['systemctl', 'status', endpoint], stdout=subprocess.PIPE).stdout.decode('utf-8').split('\n\n')[1]
        return {'success':True, 'log': log}

@app.route('/server_uptime/<endpoint>', methods=['GET'])
def get_server_uptime(endpoint):
    if request.method == 'GET':
        ...
        if debug:
            ...
            uptime = subprocess.run(['systemctl', 'status', endpoint], stdout=subprocess.PIPE).stdout.decode('utf-8').split("Active: ")[1].split(";")[0]
